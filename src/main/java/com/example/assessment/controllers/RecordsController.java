...
public class RecordsController {
    ...
    public Mono<ResponseDTO<Boolean>> borrowBook(@PathVariable Long bookId, @PathVariable Long patronId) {
        return recordService.borrowBook(bookId, patronId)
                .map(res -> new ResponseDTO<>(HttpStatus.OK.value(), res, "Book is borrowed successfully"))
    ...
    public Mono<ResponseDTO<Boolean>> returnBook(@PathVariable Long bookId, @PathVariable Long patronId) {
        return recordService.returnBook(bookId, patronId)
                .map(res -> new ResponseDTO<>(HttpStatus.OK.value(), res, "Book is returned successfully"))
                .onErrorResume(LibraryException.class,
                        ex -> Mono.just(new ResponseDTO<>(ex.getError().getStatus().value(), null, ex.getMessage())));
    }
